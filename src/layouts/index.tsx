...
const BaseLayout = () => {
  const [nav, { toggle }] = useBoolean(true);

  return (
    <section className="max-w-screen-xl h-screen mx-auto overflow-hidden relative">
      <div className="flex items-start justify-between">
        <div
          className={`h-screen hidden lg:block my-4 shadow-lg relative overflow-hidden transition-all ${
            nav ? 'w-80' : 'w-0'
          }`}
        >
          <div className="bg-primary-0 h-full rounded-2xl overflow-auto">
            ...
            <nav className="mt-6">
              <div>
                <a
                  className="w-full font-thin uppercase text-primary-500 flex items-center p-4 my-2 transition-colors duration-200 justify-start bg-gradient-to-r from-white to-primary-100 border-r-4 border-primary-500 dark:from-gray-700 dark:to-gray-800"
                  href="#"
                >
                  ...
                  <span className="mx-4 text-sm font-normal">前端导航</span>
                </a>
                {/* <a
...
// const nav = [];
// const list = document.getElementsByClassName('fe');

// for (let index = 0; index < list.length; index++) {
//   const element = list[index];
//   const type = element.getElementsByClassName('sub-category')[0].innerText.trim();
//   const links = [];
//   const children = element.getElementsByTagName('a');
//   for (let index = 0; index < children.length; index++) {
//     const child = children[index];
//     const logoHref = child.querySelector('img')?.getAttribute('src');
//     let logo;
//     main(logoHref, (base64) => {
//       logo = base64;
//       const title = child.querySelector('.logo')?.innerText.trim();
//       const desc = child.querySelector('.desc')?.innerText.trim();
...
// const str = JSON.stringify(nav, null, 2);
